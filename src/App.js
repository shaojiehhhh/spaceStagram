...
function App() {
  ...
  const fetchImagesDefault = async () => {
      //showing 5 random space's imagines when first accessing to the website
    const { data: imgs } = await axios.get(
      `https://api.nasa.gov/planetary/apod?api_key=76oMoI9O2xI73gr1U56SzDSYrYal4nFlhQ8pMLRr&count=5`
    );
    setImages(imgs);
    setIsLoading(false);
  };

  const API = `https://api.nasa.gov/planetary/apod?api_key=76oMoI9O2xI73gr1U56SzDSYrYal4nFlhQ8pMLRr&start_date=${startDateStr}&end_date=${endDateStr}`;
